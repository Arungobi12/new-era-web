        ...
        <div className="grid gap-10 lg:grid-cols-[0.75fr_1.25fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {groups.map((group) => {
              ...
              return (
                <article key={group.title} className="rounded-[1.75rem] border border-[#f6f1e8]/10 bg-[#f6f1e8]/6 p-5">
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {group.items.map((item) => (
                      <span key={item} className="rounded-full bg-[#101820] px-3 py-2 text-xs font-bold text-[#f6f1e8]/68 ring-1 ring-[#f6f1e8]/10">
                        {item}
                      </span>
                    ))}
                  </div>
